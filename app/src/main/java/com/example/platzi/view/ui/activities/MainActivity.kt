...
class MainActivity : AppCompatActivity() {
    ...
    private fun createVariableJSON(){
        ...
        val jsonArrConferences = JSONArray("[\n" +
                "            {\n" +
                "                \"datetime\" : 1564830000,\n" +
        ...
        for (i in 0 until jsonArrConferences.length()){
            val aux = jsonArrConferences.get(i) as JSONObject
            val conferences = Conferences()
            conferences.title = aux.getString("title")
            conferences.description = aux.getString("description")
            conferences.tag = aux.getString("tag")
            val calendar = Calendar.getInstance()
            calendar.timeInMillis = aux.getLong("datetime") * 1000
            conferences.dateTime = calendar.time
            conferences.speakers = aux.getString("speaker")

            fireBaseFireStore.collection("conferences").document().set(conferences)
        }
    }
}